...
FileDataSource::~FileDataSource() {
  if (file.is_open()) {
    std::cout << "Closing the file: " << filename << std::endl;
    file.close();
  }
}
...
void FileDataSource::read_data() {
    ...
    data.clear();
    double value; // empty variable
    while (file >> value) {
    // >> reads a line of file and puts it into new_val
    // push_back adds a new element to the tail of the vector
